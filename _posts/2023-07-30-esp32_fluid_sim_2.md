...
Hello from the future! It turned out that I could afford more than that. The project now performs an upscaling that is based on a particularly efficient approach to ["bilinear interpolation"](https://en.wikipedia.org/wiki/Bilinear_interpolation). This is something that I want to write more about in the future, but until then, here's a [link](https://github.com/colonelwatch/ESP32-fluid-simulation/blob/master/ESP32-fluid-simulation/ESP32-fluid-simulation.ino#L121) to the particular code that does this in the project.
...
With this post and the last post done, there's one last task to cover: the [next post](/2023/09/22/esp32_fluid_sim_3.html) is an overview of the physics before we get into the implementation. Stay tuned! But if you're here before that post comes out, there's always the code itself [on GitHub](https://github.com/colonelwatch/ESP32-fluid-simulation).
